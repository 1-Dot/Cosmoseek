...
min_size = 4
pygame.font.init()
unv_font = pygame.font.Font('assets/MiSans-Demibold.ttf', 20)
...
class Cosmoseek:
    ...
    def __init__(self):
        ...
        for _ in range(int(self.NUM_PLANETS / 10 * 1)):
        ...
        for _ in range(_ + 1, _ + 1 + int(self.NUM_PLANETS / 10 * 9)):
    ...
    def run_game(self):
        ...
        while self.rungame:
            ...
            for planet in self.planets:
                ...
                if (
                    self.is_click == True
                    and distance < max(min_size, planet.radius / planet.render_scale)
...
class Ship:
    ...
    def attempt_land(self, planet):
        if planet.typ == "terrestrial":
            ...
            self.game.message2 = f"planet.radius:{planet.radius:.2f},rewards:{planet.provisions},p_id:{planet.p_id}"
            ...
            planet.provisions = {}
            self.game.land = False
            self.game.temp_message = f"采集成功,获得{stats}"
            self.game.score += 1
            # TODO
            """
            降落成功音效
